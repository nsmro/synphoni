...
parser = argparse.ArgumentParser(formatter_class = argparse.RawDescriptionHelpFormatter,
    ...
    The orthogroup pairs are filtered using the following criterias:
        * Either 2+ children, or 1 children, 1+ sistergroup our 1 children and outgroup needs to be populated.
...
parser.add_argument("-s", "--species_tree",
                    help = "Tree of the PREFIX names extracted from chromfiles in step 1, newick format with node names\
...
parser.add_argument("-n", "--node_names", 
                    help = "spece-delimited list of nodes which will be used to filter the OG pairs\
                        evrything is run sequentially, so best launch multiple commandswith only one node",
                    required = True,
                    nargs = "+")
